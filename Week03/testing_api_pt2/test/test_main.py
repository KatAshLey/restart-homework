...
book01_unsuccess = {"name": "Percy Jackson and the Lightning Thief",
          "author": {"name": "dgrick Riordan",
                     "author_id": "RRIO-4358"},
          "year_published": 2005
}
...
def test_put_api():
    response = client.put("/books/Percy Jackson and the Lightning Thief", json=book01)
...
def test_put_api_unsuccessful():
    response = client.put("/books/Percy Jackson and the Lightning Thief",
                          headers={"Content-Type": "application/json"}, json=book01_unsuccess)
    print(response.json())
